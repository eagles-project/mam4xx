...
TEST_CASE("aero_config", "") {
  ...
  SECTION("check init") {
    ...
    for (int m=0; m<4; ++m) {
      ...
      h_diags_dry_diam[m] = Kokkos::create_mirror_view(diags.dry_geometric_mean_diameter[m]);
      h_diags_wet_diam[m] = Kokkos::create_mirror_view(diags.wet_geometric_mean_diameter[m]);
      for (int s=0; s<7; ++s) {
    ...
    for (int m=0; m<4; ++m) {
      ...
      Kokkos::deep_copy(h_diags_dry_diam[m], diags.dry_geometric_mean_diameter[m]);
      Kokkos::deep_copy(h_diags_wet_diam[m], diags.wet_geometric_mean_diameter[m]);
      for (int s=0; s<7; ++s) {
