...
Scalar init_specific_humidity(const Scalar z, const Real qv0, const Real qv1) {
...
*/
template <typename Scalar> KOKKOS_INLINE_FUNCTION
Scalar init_virtual_temperature(const Scalar z, const Real Tv0, const Real Gammav) {
  return Tv0 - Gammav * z;
}
...
*/
template <typename Scalar> KOKKOS_INLINE_FUNCTION
Scalar hydrostatic_pressure_at_height(const Scalar z, const Real p0, const Real T0,
                                    const Real Gamma) {
  Scalar result = 0;
  if (FloatingPoint<Real>::zero(Gamma)) {
    result = p0 * exp(-Constants::gravity * z /
...
*/
template <typename Scalar> KOKKOS_INLINE_FUNCTION
Scalar height_at_hydrostatic_pressure(const Scalar p, const Real p0, const Real T0,
                        const Real Gamma) {
  Scalar result = 0;
  if (FloatingPoint<Real>::zero(Gamma)) {
    result =
        -Constants::r_gas_dry_air * T0 * log(p / p0) / Constants::gravity;
  } else {
...
/** @brief Initializes a hydrostatic atmosphere column with linear
  ...
  @param [in] qv1 decay rate of water vapor [1/m]
*/
void init_atm_const_tv_lapse_rate(const Atmosphere& atm, const Real Tv0 = 300, const Real Gammav = 0.01,
  const Real qv0=0.005, const Real qv1=1e-5);
