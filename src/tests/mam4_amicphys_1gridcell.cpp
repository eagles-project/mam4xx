...
TEST_CASE("clear", "test_mam4_amicphys") {
  ...
  const Real
      ...
      {2.6516499587650101e-011 , 2.0066181180846706e-012 , 1.5918433443462643e-010 , 0.0000000000000000      },
      {3.4554314951972823e-011 , 2.7782966132370426e-012 , 1.3841809388812617e-010 , 0.0000000000000000      },
      {8.8373751603278529e-012 , 0.0000000000000000      , 5.3061041769150928e-011 , 1.8837266606981249e-012 },
      {4.6721679926790171e-013 , 0.0000000000000000      , 0.0000000000000000      , 9.4186333034906243e-011 },
      {6.8044995278829608e-011 , 6.8302276925186291e-012 , 5.4525090614694067e-010 , 0.0000000000000000      },
      {0.0000000000000000      , 0.0000000000000000      , 0.0000000000000000      , 0.0000000000000000      },
      {0.0000000000000000      , 0.0000000000000000      , 0.0000000000000000      , 0.0000000000000000      }};
  ...
  for (int i = 0; i < AeroConfig::num_modes(); ++i) {
    if (!(dgn_a[i] == Approx(check_dgn_a[i]).epsilon(epsilon)))
      std::cout << "dgn_a[i] != Approx(check_dgn_a[i])): "
                << std::setprecision(14) << dgn_a[i] << " != " << check_dgn_a[i]
                << std::endl;
    REQUIRE(dgn_a[i] == Approx(check_dgn_a[i]).epsilon(epsilon));
  }
  for (int i = 0; i < AeroConfig::num_modes(); ++i) {
    if (!(dgn_awet[i] == Approx(check_dgn_awet[i]).epsilon(epsilon)))
      std::cout << "dgn_awet[i] != Approx(check_dgn_awet[i])): "
                << std::setprecision(14) << dgn_awet[i]
                << " != " << check_dgn_awet[i] << std::endl;
    REQUIRE(dgn_awet[i] == Approx(check_dgn_awet[i]).epsilon(epsilon));
  }
  for (int i = 0; i < AeroConfig::num_modes(); ++i) {
    if (!(wetdens[i] == Approx(check_wetdens[i]).epsilon(epsilon)))
      std::cout << "wetdens[" << i << "] != Approx(check_wetdens[" << i
                << "])): " << std::setprecision(14) << wetdens[i]
                << " != " << check_wetdens[i] << std::endl;
    REQUIRE(wetdens[i] == Approx(check_wetdens[i]).epsilon(epsilon));
  }
  for (int i = 0; i < AeroConfig::num_gas_ids(); ++i) {
    const double epsilon = 0.01;
    if (!(qgas4[i] == Approx(check_qgas4[i]).epsilon(epsilon)))
      std::cout << "qgas4[" << i << "] != Approx(check_qgas4[" << i
                << "])): " << std::setprecision(14) << qgas4[i]
                << " != " << check_qgas4[i] << std::endl;
    REQUIRE(qgas4[i] == Approx(check_qgas4[i]).epsilon(epsilon));
  }
  for (int i = 0; i < AeroConfig::num_gas_ids(); ++i) {
    const double epsilon = 0.03;
    for (int j = 0; j < nqtendaa; ++j) {
      if (!(qgas_delaa[i][j] ==
            Approx(check_qgas_delaa[i][j]).epsilon(epsilon)))
  ...
  for (int i = 0; i < AeroConfig::num_modes(); ++i) {
    // qnum4 is not very close. Seems to be related to rename being
    // very sensitive to the input values and the small differences
    // between the values input to the rename process.
    const double epsilon = 1.0;
    const bool check = qnum4[i] == Approx(check_qnum4[i])
                                       .epsilon(epsilon)
                                       .margin(27000000000)
                                       .scale(20000000000);
    if (!check)
  ...
  for (int i = 0; i < AeroConfig::num_modes(); ++i) {
    // Given the volitility in the qnum base values,
    // the differences in the deltas is even larger
    // ignore
    for (int j = 0; j < nqtendaa; ++j) {
      const bool check = qnum_delaa[i][j] ==
                         Approx(check_qnum_delaa[i][j]).margin(27000000000);
      if (!check)
  ...
  for (int i = 0; i < AeroConfig::num_aerosol_ids(); ++i) {
    ...
    for (int j = 0; j < AeroConfig::num_modes(); ++j) {
      if (check_qaer4[i][j]) {
        if (!(qaer4[i][j] == Approx(check_qaer4[i][j]).epsilon(epsilon)))
          std::cout << "qaer4[" << i << "][" << j << "] != Approx(check_qaer4["
                    << i << "][" << j << "])): " << std::setprecision(14)
                    << qaer4[i][j] << " != " << check_qaer4[i][j] << std::endl;
        REQUIRE(qaer4[i][j] == Approx(check_qaer4[i][j]).epsilon(epsilon));
      }
    }
  }
  for (int i = 0; i < AeroConfig::num_aerosol_ids(); ++i) {
    const double epsilon = 1.0;
    const double scale = 1.0e-10;
    for (int j = 0; j < AeroConfig::num_modes(); ++j) {
      for (int k = 0; k < nqtendaa; ++k) {
        if (!(qaer_delaa[i][j][k] ==
  ...
  for (int i = 0; i < AeroConfig::num_modes(); ++i) {
    if (!(qwtr4[i] == Approx(check_qwtr4[i]).epsilon(epsilon)))
      std::cout << "qwtr4[" << i << "] != Approx(check_qwtr4[" << i
                << "])): " << std::setprecision(14) << qwtr4[i]
                << " != " << check_qwtr4[i] << std::endl;
    REQUIRE(qwtr4[i] == Approx(check_qwtr4[i]).epsilon(epsilon));
  }
}
