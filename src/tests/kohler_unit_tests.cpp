...
struct KohlerSolveTestFtor {
  typedef mam4::DeviceType::view_1d<Real> RealView;
  typedef mam4::DeviceType::view_1d<int> IntView;
  ...
  void operator()(const int i) const {
    mam4::KohlerSolver<mam4::math::NewtonSolver<mam4::KohlerPolynomial>>
        newton_solver(relative_humidity(i), hygroscopicity(i), dry_radius(i),
                      tol);
    mam4::KohlerSolver<mam4::math::BisectionSolver<mam4::KohlerPolynomial>>
        bisection_solver(relative_humidity(i), hygroscopicity(i), dry_radius(i),
                         tol);
    mam4::KohlerSolver<
        mam4::math::BracketedNewtonSolver<mam4::KohlerPolynomial>>
    ...
    newton_err(i) = mam4::abs(newton_sol(i) - true_sol(i));
    bisection_err(i) = mam4::abs(bisection_sol(i) - true_sol(i));
    bracket_err(i) = mam4::abs(bracket_sol(i) - true_sol(i));
...
TEST_CASE("kohler_physics_functions", "") {
  ...
  const Real mam4_surften = mam4::Constants::surface_tension_h2o_air_273k;
  const Real mam4_kelvin_a = mam4::kelvin_coefficient();
  ...
  for (int i = 0; i <= nn; ++i) {
    ...
    const Real sigma = mam4::surface_tension_water_air(T);
    const Real k_a = mam4::kelvin_coefficient(T);
  ...
  REQUIRE(mam4::kelvin_coefficient() * 1e6 ==
          Approx(0.00120746723156361711).epsilon(7e-3));
  REQUIRE(mam4::surface_tension_water_air() ==
...
TEST_CASE("kohler_verificiation", "") {
  ...
  mam4::KohlerVerification verification(N);

  SECTION("polynomial_properties") {
    mam4::DeviceType::view_1d<Real> k_of_zero("kohler_poly_zero_input", N3);
    mam4::DeviceType::view_1d<Real> k_of_rdry("kohler_poly_rdry_input", N3);
    mam4::DeviceType::view_1d<Real> k_of_25rdry("kohler_poly_25rdry_input", N3);
    ...
    Kokkos::parallel_for(
        "KohlerVerification::test_properties", N3, KOKKOS_LAMBDA(const int i) {
          const Real mam4_default_temperature = mam4::Constants::triple_pt_h2o;
          const auto kpoly = mam4::KohlerPolynomial(rh(i), hyg(i), rdry(i),
    ...
    const Real mam4_kelvin_a = mam4::kelvin_coefficient() * 1e6;

    for (int i = 0; i < N3; ++i) {
      REQUIRE(mam4::FloatingPoint<Real>::equiv(
          h_k0(i), mam4_kelvin_a * mam4::cube(h_rdry(i))));
      REQUIRE(h_krdry(i) > 0);
      REQUIRE(h_k25(i) < 0);
    }
  }
  ...
  SECTION("polynomial_roots") {
    ...
    mam4::DeviceType::view_1d<Real> newton_sol("kohler_newton_sol", N3);
    mam4::DeviceType::view_1d<Real> newton_err("kohler_newton_err", N3);
    mam4::DeviceType::view_1d<int> newton_iterations("kohler_newton_iterations",
                                                     N3);
    mam4::DeviceType::view_1d<Real> bisection_sol("kohler_bisection_sol", N3);
    mam4::DeviceType::view_1d<Real> bisection_err("kohler_bisection_err", N3);
    mam4::DeviceType::view_1d<int> bisection_iterations(
        "kohler_bisection_iterations", N3);
    mam4::DeviceType::view_1d<Real> bracket_sol("kohler_bracket_sol", N3);
    mam4::DeviceType::view_1d<Real> bracket_err("kohler_bracket_err", N3);
    mam4::DeviceType::view_1d<int> bracket_iterations(
        "kohler_bracket_iterations", N3);
