...
void set_subarea_qnumb_for_cldbrn_aerosols(Ensemble *ensemble) {
  ensemble->process([=](const Input &input, Output &output) {
    ...
    std::string input_arrays[] = {"fcldy",       "jcldy",      "jclea",
                                  "loffset",     "maxsubarea", "ncnst",
                                  "nspec_amode", "ntot_amode", "numptrcw_amode",
                                  "qqcwgcm",     "qqcwsub"};
    ...
    Kokkos::parallel_for(
        team_policy, KOKKOS_LAMBDA(const ThreadTeam &team) {
          ...
          for (int j = 0; j < subarea_max; ++j) {
            for (int i = 0; i < gas_pcnst; ++i) {
              qqcwsub[i][j] = qqcwsub_d(i, j);
            }
          }
          mam4::microphysics::set_subarea_qnumb_for_cldbrn_aerosols(
