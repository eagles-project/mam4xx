...
void ma_precpprod(Ensemble *ensemble) {
  ...
  ensemble->process([=](const Input &input, Output &output) {
    ...
    std::vector<Real> rprd_host, dpdry_i_host, dcondt_wetdep_host, doconvproc_extd_host,species_class_host, wd_flux_host, dcondt_host, dcondt_prevap_host, dcondt_prevap_hist_host, mmtoo_prevap_resusp_host ;
    ColumnView rprd_dev, dpdry_i_dev, doconvproc_extd_dev, species_class_dev, wd_flux_dev, mmtoo_prevap_resusp_dev;
    get_input(input, "rprd", nlev, rprd_host, rprd_dev);
    get_input(input, "dpdry_i", nlev, dpdry_i_host, dpdry_i_dev);
    Kokkos::View<Real **, Kokkos::MemoryUnmanaged> dcondt_wetdep_dev, dcondt_dev, dcondt_prevap_dev, dcondt_prevap_hist_dev;
    get_input(input, "dcondt_wetdep", nlev, pcnst_extd, dcondt_wetdep_host,
              dcondt_wetdep_dev);
    get_input(input, "species_class", ConvProc::gas_pcnst, species_class_host, species_class_dev);
    ...
    get_input(input, "dcondt", nlev, pcnst_extd, dcondt_host,
              dcondt_dev);
    get_input(input, "dcondt_prevap", nlev, pcnst_extd, dcondt_prevap_host,
              dcondt_prevap_dev);
    get_input(input, "dcondt_prevap_hist", nlev, pcnst_extd, dcondt_prevap_hist_host,
              dcondt_prevap_hist_dev);
    get_input(input, "mmtoo_prevap_resusp", ConvProc::gas_pcnst, mmtoo_prevap_resusp_host, mmtoo_prevap_resusp_dev);
    ...
    Kokkos::parallel_for(
        "ma_precpprod", 1, KOKKOS_LAMBDA(int) {
          ...
          convproc::ma_precpprod(
            rprd[kk], dpdry_i[kk], doconvproc_extd, x_ratio,
            species_class, mmtoo_prevap_resusp, 
            flux, flux_tmp, flux_base, 
            wd_flux, dcondt_wetdep, dcondt, dcondt_prevap,dcondt_prevap_hist);
          return_vals[0] = flux;
    ...
    set_output(output, "dcondt", nlev, pcnst_extd, dcondt_host,
              dcondt_dev);
    set_output(output, "dcondt_prevap", nlev, pcnst_extd, dcondt_prevap_host,
              dcondt_prevap_dev);
    set_output(output, "dcondt_prevap_hist", nlev, pcnst_extd, dcondt_prevap_hist_host,
              dcondt_prevap_hist_dev);
  });
  ...
  EKAT_REQUIRE_MSG(dcondt == 1+2*(ConvProc::pcnst_extd-1), "Special case of input failed.");
  EKAT_REQUIRE_MSG(dcondt_prevap == 1+2*(ConvProc::pcnst_extd-1), "Special case of input failed.");
  EKAT_REQUIRE_MSG(dcondt_prevap_hist == 1+2*(ConvProc::pcnst_extd-1), "Special case of input failed.");
}
