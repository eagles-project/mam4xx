...
void mam_rename_1subarea(Ensemble *ensemble) {
  ...
  ensemble->process([=](const Input &input, Output &output) {
    ...
    constexpr int naerosol_species = mam4::AeroConfig::num_aerosol_ids();
    ...
    mam4::validation::convert_vector_to_modal_array(qnum_cur_vector, qnum_cur);
    ...
    if (iscloudy) {
      ...
      mam4::validation::convert_vector_to_modal_array(qnumcw_cur_vector,
                                                      qnumcw_cur);

      auto qaercw_cur_vector = input.get_array("qaercw_cur");
      mam4::validation::convert_vector_to_mass_mixing_ratios(qaercw_cur_vector,
                                                             qaercw_cur);

      auto qaercw_del_grow4rnam_vector =
          input.get_array("qaercw_del_grow4rnam");
      mam4::validation::convert_vector_to_mass_mixing_ratios(
    ...
    mam4::validation::convert_vector_to_mass_mixing_ratios(
        qaer_del_grow4rnam_vector, qaer_del_grow4rnam);

    auto qaer_cur_vector = input.get_array("qaer_cur");
    Real qaer_cur[nmodes][naerosol_species];
    mam4::validation::convert_vector_to_mass_mixing_ratios(qaer_cur_vector,
                                                           qaer_cur);

    mam4::Rename this_rename;
    ...
    mam4::validation::convert_mass_mixing_ratios_to_vector(qaer_cur,
                                                           qaer_cur_out);
    output.set("qaer_cur", qaer_cur_out);

    if (iscloudy) {
      std::vector<Real> qnumcw_cur_out(nmodes + 1, 0);
      mam4::validation::convert_modal_array_to_vector(qnumcw_cur,
                                                      qnumcw_cur_out);
      output.set("qnumcw_cur", qnumcw_cur_out);

      std::vector<Real> qaercw_cur_out((nmodes + 1) * naerosol_species, 0);
      mam4::validation::convert_mass_mixing_ratios_to_vector(qaercw_cur,
                                                             qaercw_cur_out);
