...
void find_renaming_pairs(Ensemble *ensemble) {
  ...
  ensemble->process([=](const Input &input, Output &output) {
    ...
    sz_factor[1] =
        mam4::Constants::pi_sixth *
        mam4::exp(4.5 * mam4::square(mam4::log(mam4::modes(1).mean_std_dev)));
    sz_factor[0] =
        mam4::Constants::pi_sixth *
        mam4::exp(4.5 * mam4::square(mam4::log(mam4::modes(0).mean_std_dev)));

    std::vector<Real> sz_factor_out(nmodes, 0);
    mam4::validation::convert_modal_array_to_vector(sz_factor, sz_factor_out);
    output.set("sz_factor", sz_factor_out);

    std::vector<Real> fmode_dist_tail_fac_out(nmodes, 0);
    mam4::validation::convert_modal_array_to_vector(fmode_dist_tail_fac,
                                                    fmode_dist_tail_fac_out);
    output.set("fmode_dist_tail_fac", fmode_dist_tail_fac_out);

    std::vector<Real> v2n_lo_rlx_out(nmodes, 0);
    mam4::validation::convert_modal_array_to_vector(v2n_lo_rlx, v2n_lo_rlx_out);
    output.set("v2n_lo_rlx", v2n_lo_rlx_out);

    std::vector<Real> v2n_hi_rlx_out(nmodes, 0);
    mam4::validation::convert_modal_array_to_vector(v2n_hi_rlx, v2n_hi_rlx_out);
    output.set("v2n_hi_rlx", v2n_hi_rlx_out);

    std::vector<Real> ln_diameter_tail_fac_out(nmodes, 0);
    mam4::validation::convert_modal_array_to_vector(ln_diameter_tail_fac,
    ...
    mam4::validation::convert_modal_array_to_vector(diameter_cutoff,
                                                    diameter_cutoff_out);
    ...
    mam4::validation::convert_modal_array_to_vector(ln_dia_cutoff,
                                                    ln_dia_cutoff_out);
    output.set("ln_dia_cutoff", ln_dia_cutoff_out);

    std::vector<Real> diameter_threshold_out(nmodes, 0);
    mam4::validation::convert_modal_array_to_vector(diameter_threshold,
