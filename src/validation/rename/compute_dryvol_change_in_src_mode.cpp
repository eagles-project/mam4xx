...
void compute_dryvol_change_in_src_mode(Ensemble *ensemble) {
  ...
  ensemble->process([=](const Input &input, Output &output) {
    ...
    mam4::validation::convert_vector_to_mass_mixing_ratios(q_mmr_vector, q_mmr);

    auto q_del_growth_vector = input.get_array("q_del_growth");
    Real q_del_growth[nmodes][naerosol_species];
    mam4::validation::convert_vector_to_mass_mixing_ratios(q_del_growth_vector,
                                                           q_del_growth);
    ...
    mam4::validation::convert_modal_array_to_vector(dryvol, dryvol_out);
    output.set("dryvol", dryvol_out);

    std::vector<Real> deldryvol_out(nmodes, 0);
    mam4::validation::convert_modal_array_to_vector(deldryvol, deldryvol_out);
