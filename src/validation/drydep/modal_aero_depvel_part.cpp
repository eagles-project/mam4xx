...
void modal_aero_depvel_part(Ensemble *ensemble) {
  ...
  ensemble->process([=](const Input &input, Output &output) {
    ...
    mam4::ColumnView vlc_trb = mam4::validation::create_column_view(nlev);
    mam4::ColumnView vlc_dry = mam4::validation::create_column_view(nlev);
    ...
    Kokkos::parallel_for(
        "modal_aero_depvel_part", nlev, KOKKOS_LAMBDA(int lev) {
          ...
          mam4::drydep::modal_aero_depvel_part(
