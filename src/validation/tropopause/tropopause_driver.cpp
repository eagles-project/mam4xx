...
void usage() {
  std::cerr << "tropopause_driver: a Skywalker driver for validating the "
               "MAM4 rename parameterizations."
            << std::endl;
  std::cerr << "tropopause_driver: usage:" << std::endl;
  std::cerr << "tropopause_driver <input.yaml>" << std::endl;
  exit(0);
}
