...
void compute_dry_volume_k(Ensemble *ensemble) {
  ...
  ensemble->process([=](const Input &input, Output &output) {
    ...
    mam4::DeviceType::view_1d<Real> dryvol_i("Return dryvol_i", 1);
    mam4::DeviceType::view_1d<Real> dryvol_c("Return dryvol_c", 1);
    ...
    Kokkos::parallel_for(
        "compute_dry_volume_k", 1, KOKKOS_LAMBDA(int k) {
          ...
          for (int ispec = 0; ispec < n_spec; ispec++) {
            const mam4::AeroId aero_id = mam4::mode_aero_species(imode, ispec);
          ...
          Real dryvol_i_k = 0;
          Real dryvol_c_k = 0;
          mam4::calcsize::compute_dry_volume_k(k, imode, inv_density,
                                               progs,      // in
                                               dryvol_i_k, // out
                                               dryvol_c_k);
