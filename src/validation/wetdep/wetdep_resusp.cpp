...
void test_wetdep_resusp_process(const Input &input, Output &output) {
  ...
  std::string input_variables[] = {
      "dt",          "is_st_cu",    "mam_prevap_resusp_optcc",
      "pdel_ik",     "evapx",       "precabx_base_old",
  ...
  const Real evapx = input.get("evapx");
  ...
  ColumnView return_vals = mam4::validation::create_column_view(5);
  Kokkos::parallel_for(
      "wetdep::wetdep_resusp", 1, KOKKOS_LAMBDA(const int) {
        ...
        mam4::wetdep::wetdep_resusp(
            is_st_cu, mam_prevap_resusp_optcc, pdel_ik, evapx, precabx_old,
            precabx_base_old, scavabx_old, precnumx_base_old, precabx_new,
...
void test_wetdep_resusp(std::unique_ptr<Ensemble> &ensemble) {
  ensemble->process([&](const Input &input, Output &output) {
    test_wetdep_resusp_process(input, output);
  });
}
