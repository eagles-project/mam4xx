...
void mode_hygroscopicity_i(const AeroSpeciesView &aero_species,
                           const Diagnostics &diags, const Prognostics &progs,
                           int mode_idx, int k) {
  Real hyg = 0.0;
...
///  @param [in] k Column vertical level where size data are needed
KOKKOS_INLINE_FUNCTION
void mode_hygroscopicity(const AeroSpeciesView &aero_species,
                         const Diagnostics &diags, const Prognostics &progs,
                         int k) {
  for (int m = 0; m < AeroConfig::num_modes(); ++m) {
    mode_hygroscopicity_i(aero_species, diags, progs, m, k);
  }
}
