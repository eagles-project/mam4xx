...
void calculate_resistance_rclx(
  ...
  void calculate_resistance_rlux(
      const int beglt, const int endlt, const int index_season[n_land_type],
      const bool fr_lnduse[n_land_type], const bool has_rain,
      const bool has_dew,
      ...
      const Real cts,           // correction to rlu rcl and rgs for frost
      Real rlux[gas_pcnst]
               [n_land_type]) { // lower canopy resistance [s/m] ! out
  ...
  Real get_saturation_specific_humidity(const Real temperature, // [K]
    ...
    use physconst, only
        : tmelt
              ...
              Real es = 611.0 * haero::exp(5414.77 * (temperature - tmelt) /
                                           (tmelt * temperature));
  ...
  void drydep_xactive(
      ...
      const int
          col_index_season[12],  // column-specific mapping of month indices to
    ...
    calculate_ustar(0, n_land_type, index_season, fr_lnduse, unstable, zl,
                    uustar, ribn, ustar, cvar, bycp);

    calculate_ustar_over_water(0, n_land_type, index_season, fr_lnduse,
                               unstable, zl, uustar, ribn, ustar, cvar, bycp);

    calculate_obukhov_length(0, n_land_type, fr_lnduse, unstable, tha, thg,
    ...
    calculate_resistance_rclx(0, n_land_type, index_season, fr_lnduse, heff,
                              cts, rclx);
    ...
    calculate_gas_drydep_vlc_and_flux(
        0, n_land_type, index_season, fr_lnduse, lcl_frc_landuse, mmr, dep_ra,
        dep_rb, term, rsmx, rlux, rclx, rgsx, rdc, dvel, dflx);
  }
