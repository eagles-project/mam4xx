...
void calculate_uustar(const int index_season[n_land_type],
  ...
  if(unstable) {
    ...
    ustarb = cvarb * va * haero::sqrt(1.0 - (9.4*ribn/(1.0 + 7.4*bb))); // BAD_CONSTANTS
  } else {
    ustarb = cvarb * va(icol)/(1. + 4.7*ribn)
  }
  uustar = va * ustarb;
}

KOKKOS_INLINE_FUNCTION
void calculate_ustar(const int beglt, const int endlt,
                     const int index_season[n_land_type],
                     const bool fr_lnduse[n_land_type],
                     const bool unstable,
...
KOKKOS_INLINE_FUNCTION
void calculate_ustar_over_water(const int beglt, const int endlt,
                                const int index_season[n_land_type],
                                const bool fr_lnduse[n_land_type],
                                const bool unstable,
                                const Real zl,            // height of lowest level [m]
  ...
  if (fr_lnduse[lt]) {
    // BAD_CONSTANTS
    Real z0water = 0.016*haero::square(ustar[lt])/grav + diffk/(9.1*ustar[lt]);
    cvar[lt]     = karman/haero::log(zl/z0water);
    if(unstable) {
      bycp[lt]  = 9.4*haero::square(cvar[lt])*haero::sqrt(haero::abs(ribn)*zl/z0water);
      ustar[lt] = haero::sqrt(cvar[lt]*uustar*haero::sqrt(1.0 - (9.4*ribn/(1.0 + 7.4*bycp[lt]))));
    } else {
      ustar[lt] = haero::sqrt(cvar[lt]*uustar/(1.0 + 4.7*ribn));
    }
...
//-------------------------------------------------------------------------------------
KOKKOS_INLINE_FUNCTION
void calculate_obukhov_length(const int beglt, const int endlt,
                              const bool fr_lnduse[n_land_type],
                              const bool unstable,
                              ...
                              const Real bycp, // buoyancy parameter for unstable conditions
                              const Real ribn, // richardson number [unitless]
...
KOKKOS_INLINE_FUNCTION
void calculate_aerodynamic_and_quasilaminar_resistance(const int beglt, const int endlt,
                                                       const bool fr_lnduse[n_land_type],
                                                       const Real zl, // height of lowest level [m]
  ...
  for (int lt = beglt; lt < endlt; ++lt) {
    if (fr_lnduse[lt]) {
      ...
      if (obklen[lt] < 0.0) {
        ...
        psih = haero::exp(0.598 + 0.39*haero::log(-zeta) - 0.09*haero::square(haero::log(-zeta)));
      } else {
        Real zeta = zl/obklen[lt];
...
KOKKOS_INLINE_FUNCTION
void calculate_resistance_rgsx_and_rsmx(const int beglt, const int endlt,
                                        const int index_season[n_land_type],
                                        const bool fr_lnduse[n_land_type],
                                        const bool has_rain,
                                        const bool has_dew,
                                        ...
                                        Real rsmx[gas_pcnst][n_land_type]) { // vegetative resistance (plant mesophyll) [s/m]
    ...
    if(has_dvel[ispec]) {
      ...
      for (int lt = beglt; lt < endlt; ++lt) {
        if (fr_lnduse[lt]) {
          ...
          rgsx[ispec][lt] = cts + 1.0/((heff[idx_drydep]/(1e5*rgss[sndx][lt])) + (foxd[idx_drydep]/rgso[sndx][lt]));
...
KOKKOS_INLINE_FUNCTION
void calculate_resistance_rclx(const int beglt, const int endlt,
                               const int index_season[n_land_type],
                               const bool fr_lnduse[n_land_type],
                               const Real heff[nddvels], //Henry's law coefficients
      ...
      for (int lt = beglt; lt < endlt; ++lt) {
        if (fr_lnduse[lt]) {
          if (lt == lt_for_water) {
            ...
            rclx[ispec][lt] = cts + 1.0/((heff[idx_drydep]/(1e5*rcls[sndx][lt])) + (foxd[idx_drydep]/rclo[sndx][lt]));
          }
        }
      }
    }
...
KOKKOS_INLINE_FUNCTION
void calculate_resistance_rlux(const int beglt, const int endlt,
                               const int index_season[n_land_type],
                               const bool fr_lnduse[n_land_type],
                               const bool has_rain,
                               const bool has_dew,
  ...
  // FIXME: use physconst,      only: tmelt
      ...
      for (int lt = beglt; lt < endlt; ++lt) {
        if (fr_lnduse[lt]) {
          if (lt == lt_for_water) {
            ...
            rlux[ispec][lt] = cts + rlu[sndx][lt]/(1e-5*heff[idx_drydep] + foxd[idx_drydep]);
          }
        }
      }
    }
    ...
    if (lt != lt_for_water) {
      if (fr_lnduse[lt]) {
        ...
        if (sfc_temp > tmelt) {
          ...
          if (has_rain) {
            rlux_o3[lt] = 3000.0*rlu[sndx][lt]/(1000.0 + 3.0*rlu[sndx][lt]);
          }
        }
      }
    }
        ...
        if (lt != lt_for_water) {
          if (fr_lnduse[lt] && (sfc_temp > tmelt) && has_dew) {
            ...
            rlux[ispec][lt] = 1.0/((1.0/(3.*rlux[ispec][lt]))
                                  + 1e-7*heff[idx_drydep] + foxd[idx_drydep]/rlux_o3[lt]);
          }
        }
      }
        ...
        if (lt != lt_for_water) {
          if (fr_lnduse[lt]) {
            ...
            if (sfc_temp > tmelt) {
              ...
              if (has_rain) {
                // FIXME: rlu probably needs its indices swapped
                rlux[ispec][lt] = 15.0*rlu[index_season[lt]][lt]/(5.0 + 3e-3*rlu[index_season[lt]][lt]);
              }
            }
            rlux[ispec][lt] += cts;
          }
...
KOKKOS_INLINE_FUNCTION
void calculate_gas_drydep_vlc_and_flux(const int beglt, const int endlt,        // land type index endpoints
                                       const int index_season[n_land_type],
                                       const bool fr_lnduse[n_land_type],
                                       const Real lcl_frc_landuse[n_land_type],
                                       ...
                                       const Real term,
                                       const Real rsmx[gas_pcnst][n_land_type], // vegetative resistance (plant mesophyll) [s/m]
                                       const Real rlux[gas_pcnst][n_land_type], // vegetative resistance (upper canopy) [s/m]
...
Real get_potential_temperature(const Real temperature,         // [K]
  ...
  // BAD_CONSTANT
  return temperature * haero::pow(p00/pressure, rovcp) * (1.0 + 0.61*specific_humidity);
}
...
Real get_saturation_specific_humidity(const Real temperature, // [K]
  ...
  Real es = 611.0 * haero::exp(5414.77*(temperature - tmelt)/(tmelt*temperature));
...
void drydep_xactive(const int ncdate,               // date [YYMMDD]
  ...
  //-------------------------------------------------------------------------------------
  // BAD_CONSTANTS
  Real zl = -rair/grav * air_temp * (1. + 0.61*spec_hum) * log(pressure_10m/pressure_sfc);
  ...
  calculate_aerodynamic_and_quasilaminar_resistance(0, n_land_type, fr_lnduse,
                                                    zl, obklen, ustar, cvar,
                                                    dep_ra, dep_rb);
