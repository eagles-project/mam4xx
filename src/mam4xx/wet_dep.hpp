  ...
  */
  ...
  if (mam_prevap_resusp_optcc <= 130) {
    ...
    if (precabx_base_new < small_value_30) {
      ...
      const Real tmpa = arainx * flux_precnum_vs_flux_prec_mpln(precabx_base_new / arainx,
                                                     is_st_cu);
...
void wetdep_resusp(const int is_st_cu, const int mam_prevap_resusp_optcc,
  ...
  */
  ...
  if (precabx_new < small_value_30) {
    ...
    if (mam_prevap_resusp_optcc <= 130) scavabx_new = 0.0;
...
{
const Real gravit = Constants::gravity;
precabx_base_tmp = precabs_base;
const Real tmps = haero::max(0.0, evaps * pdel / gravit);
precabx_tmp = utils::min_max_bound(0.0, precabx_base_tmp, precabs - tmps);
}
{
  scavabx_tmp = scavabs;
  precnumx_base_tmp = precnums_base;
}
...
{   
const Real gravit = Constants::gravity;
const Real tmps = haero::max(0.0, precs * pdel / gravit);
precabs_base = haero::max(0.0, precabx_base_tmp + tmps);
precabs = utils::min_max_bound(0.0, precabs_base, precabx_tmp + tmps);
}
    wetdep_prevap(1, mam_prevap_resusp_optcc, arainx,
                  precabx_base_tmp, precnumx_base_tmp,
                  precabs_base, precnums_base);
{
scavabs = scavabx_tmp;
if (mam_prevap_resusp_optcc <= 130) {
  const Real gravit = Constants::gravity;
  const Real tmpa = haero::max(0.0, srcs * pdel / gravit);
  scavabs = haero::max(0.0, scavabx_tmp + tmpa);
}
}
    // for convective clouds
{
const Real gravit = Constants::gravity;
precabx_base_tmp = precabc_base;
const Real tmps = haero::max(0.0, evapc * pdel / gravit);
precabx_tmp = utils::min_max_bound(0.0, precabx_base_tmp, precabc - tmps);
}
{
  scavabx_tmp = scavabc;
  precnumx_base_tmp = precnumc_base;
}
...
{    
const Real gravit = Constants::gravity;
Real tmpc = haero::max(0.0, cmfdqr * pdel / gravit);
precabc_base = haero::max(0.0, precabx_base_tmp + tmpc);
precabc = utils::min_max_bound(0.0, precabc_base, precabx_tmp + tmpc);
}

    wetdep_prevap(2, mam_prevap_resusp_optcc, arainx,
                  precabx_base_tmp, precnumx_base_tmp,
                  precabc_base, precnumc_base);
{
scavabc = scavabx_tmp;
if (mam_prevap_resusp_optcc <= 130) {
  const Real gravit = Constants::gravity;
  const Real tmpa = haero::max(0.0, srcs * pdel / gravit);
  scavabc = haero::max(0.0, scavabx_tmp + tmpa);
}
}
...
if (mam_prevap_resusp_optcc == 0) {
  const Real gravit = Constants::gravity;
  scavabs = scavabs * (1 - fracev_st) + srcs * pdel / gravit;
  scavabc = scavabc * (1 - fracev_cu) + srcc * pdel / gravit;
  precabs = precabs + (precs - evaps) * pdel / gravit;
  precabc = precabc + (cmfdqr - evapc) * pdel / gravit;
}
...
void compute_q_tendencies(
  ...
  Kokkos::single(Kokkos::PerTeam(team), [=]() {
    ...
    Real precabs = 0;
    Real precabs_vec[nlev] = {};
    Real precabc = 0;
    Real precabc_vec[nlev] = {};

    Real scavabs = 0;
    Real scavabs_vec[nlev] = {};
    Real scavabc = 0;
    Real scavabc_vec[nlev] = {};

    Real precabs_base = 0;
    Real precabs_base_vec[nlev] = {};
    Real precabc_base = 0;
    Real precabc_base_vec[nlev] = {};
    Real precnums_base = 0;
    Real precnums_base_vec[nlev] = {};
    Real precnumc_base = 0;
    Real precnumc_base_vec[nlev] = {};

    Real precabx_base_tmp[nlev] = {};
    Real precabs_tmp_vec[nlev] = {};
    ...
    for (int k = 0; k < nlev; ++k) {
      const Real gravit = Constants::gravity;
      tmpa[k] = haero::max(0.0, evapr[k] * pdel[k] / gravit);
      tmpb[k] = haero::max(0.0, prain[k] * pdel[k] / gravit);
      tmpc[k] = (prain[k] - evapr[k]) * pdel[k] / gravit;
    }
    if (mam_prevap_resusp_optcc >= 100) {
      for (int k = 0; k < nlev-1; ++k) {
        const Real small_value_30 = 1.e-30;
        Real precabs_tmp = utils::min_max_bound(0.0, precabs_base_vec[k], precabs_vec[k] - tmpa[k]); 
	precabs_tmp_vec[k] = precabs_tmp;
        precabx_base_tmp[k] = precabs_base_vec[k];
        if (precabs_tmp < small_value_30) {
  	  // setting both these precip rates to zero causes the resuspension
          // calculations to start fresh if there is any more precip production
	  precabx_base_tmp[k] = 0;
	  precabs_tmp = 0;
        }
        precabs_base_vec[k+1] = haero::max(0.0, precabx_base_tmp[k] + tmpb[k]);

        precabs_vec[k+1] = utils::min_max_bound(0.0, precabs_base_vec[k+1], precabs_tmp + tmpb[k]);
      }
    } else if (mam_prevap_resusp_optcc == 0) 
      for (int k = 0; k < nlev-1; ++k) {
        precabs_vec[k+1] = precabs_vec[k] + tmpc[k];
    }

    if (mam_prevap_resusp_optcc >= 100 && mam_prevap_resusp_optcc > 130) {
      // This can be done as three parallel loops, push the loop into the conditional
      for (int k = 0; k < nlev-1; ++k) {
        const Real small_value_2 = 1.e-2;
        const Real small_value_30 = 1.e-30;
        // raindrop number increase
	//
        if (precabs_base_vec[k+1] < small_value_30) {
          precnums_base_vec[k+1] = 0.0;
        } else if (precabs_base_vec[k+1] > precabx_base_tmp[k]) {
          // note - calc rainshaft number flux from rainshaft water flux,
          // then multiply by rainshaft area to get grid-average number flux
          const Real arainx = haero::max(cldvst[k+1], small_value_2);
          const Real tmpa = arainx * flux_precnum_vs_flux_prec_mpln(precabs_base_vec[k+1] / arainx, 1);
          precnums_base_vec[k+1] = haero::max(0.0, tmpa);
        } else {
          precnums_base_vec[k+1] = precnums_base_vec[k];
        }
      }
    }
    ...
    Real srcc[nlev] = {};
    const bool is_strat_cloudborne = lphase == 1 ? false : true;
    for (int k = 0; k < nlev; ++k) {
      const Real small_value_36 = 1.e-36;
      const Real small_value_12 = 1.e-12;
      const Real small_value_5 = 1.e-5; 
      const Real small_value_2 = 1.e-2; 
      const bool is_st_cu = 2;
      Real scavcoef = 0;
      if (jnv) 
        scavcoef = (1 == jnv) ? scavcoefnum[k] : scavcoefvol[k];

      const Real tqqcw = lphase == 1 ? qqcw(k, mm) : 0.0;
      const Real tracer = lphase == 1 ? state_q(k,mm) + ptend_q(k,mm) * dt : qqcw(k, mm); 
      Real fracp =
          cmfdqr[k] * dt /
          haero::max(small_value_12, cldcu[k] * conicw[k] + (cmfdqr[k] + dlf[k]) * dt);
      ...
      // temporarily calculation of tracer [kg/kg]
      const Real tracer_tmp = haero::min(
          tqqcw, tracer * (clddiff / haero::max(small_value_2, (1. - clddiff))));
      // calculate in-cumulus and mean tracer values for wetdep_scavenging use
      // in-cumulus tracer concentration [kg/kg]
      const Real tracer_incu = f_act_conv[k] * (tracer + tracer_tmp);
      // mean tracer concenration [kg/kg]
      Real tracer_mean =
          tracer * (1. - cldcu[k] * f_act_conv[k]) - cldcu[k] * f_act_conv[k] * tracer_tmp;
      tracer_mean = haero::max(0., tracer_mean);

      // calculate limitation of removal rate using Dana and Hales coefficient
      // odds  : limit on removal rate (proportional to prec) [fraction]
      Real odds = precabc_vec[k] / haero::max(cldvcu[k], small_value_5) * scavcoef * dt;
      odds = utils::min_max_bound(0.0, 1.0, odds);

      Real src1 = 0; // incloud scavenging tendency [kg/kg/s]
      Real src2 = 0; // below-cloud scavenging tendency [kg/kg/s]
      if (is_strat_cloudborne) {
        if (is_st_cu == 2) {
          // convective cloud does not affect strat-cloudborne aerosol
          src1 = 0;
          ...
          // strat in-cloud removal only affects strat-cloudborne aerosol
          // in-cloud scavenging:
          src1 = sol_factic[k] * fracp * tracer_incu / dt;
        }
       // no below-cloud scavenging for strat-cloudborne aerosol
       src2 = 0;
      } else {
        ...
        src2 = sol_factb[k] * cldvcu[k] * odds * tracer_mean / dt;
      }
      srcc[k] = src1 + src2; // total stratiform or convective scavenging
    }
    ...
    for (int k = 0; k < nlev; ++k) {
      ...
      Real fracp = prain[k] * dt / haero::max(totcond[k] + prain[k] * dt, small_value_12);
      fracp = utils::min_max_bound(0.0, 1.0, fracp);
      ...
      Real tracer_mean =
       ...
       // calculate limitation of removal rate using Dana and Hales coefficient
       // odds  : limit on removal rate (proportional to prec) [fraction]
       Real odds = precabs_vec[k] / haero::max(cldvst[k], small_value_5) * scavcoef * dt;
       odds = utils::min_max_bound(0.0, 1.0, odds);
      ...
      if (is_strat_cloudborne) {
        if (is_st_cu == 2) {
          ...
          // strat in-cloud removal only affects strat-cloudborne aerosol
          // in-cloud scavenging:
          src1 = sol_facti[k] * fracp * tracer / dt;
       }
       // no below-cloud scavenging for strat-cloudborne aerosol
       src2 = 0;
      } else {
        ...
        src2 = sol_factb[k] * cldvst[k] * odds * tracer_mean / dt;
      }
    ...
    for (int k = 0; k < nlev; ++k) {
      const Real small_value_36 = 1.e-36;
      const Real tracer = lphase == 1 ? state_q(k,mm) + ptend_q(k,mm) * dt : qqcw(k, mm); 
      const Real rat = tracer / haero::max(dt * (srcc[k] + srcs[k]), small_value_36);
      if (rat < 1) {
       srcs[k] = srcs[k] * rat;
       srcc[k] = srcc[k] * rat;
      }
    ...
    for (int k = 0; k < nlev; ++k) {
      const Real gravit = Constants::gravity;
      tmpd[k] = haero::max(0.0, srcs[k] * pdel[k] / gravit);
    }
    for (int k = 0; k < nlev-1; ++k) {
      const Real gravit = Constants::gravity;
      const int is_st_cu = 1;
      const Real small_value_30 = 1.e-30;
      Real scavabs = scavabs_vec[k];
      if (mam_prevap_resusp_optcc >= 100) {
      scavabs = scavabs_vec[k];
      if (precabs_tmp_vec[k] < small_value_30) {
	if (mam_prevap_resusp_optcc <= 130) 
	  scavabs = 0.0;
      } else if (evapr[k] <= 0.0) {
       // no evap so no resuspension
       if (mam_prevap_resusp_optcc <= 130) 
        ...
        if (mam_prevap_resusp_optcc <= 130) {
          // fraction of precabx and precabx_base
          const Real u_old =
              utils::min_max_bound(0.0, 1.0, precabs_vec[k] / precabs_base_vec[k]);
          // fraction after calling function *_resusp_vs_fprec_evap_mpln
          Real x_old = 0, x_new = 0;
          if (mam_prevap_resusp_optcc <= 130) {
            // non-linear resuspension of aerosol mass
            x_old = 1.0 - faer_resusp_vs_fprec_evap_mpln(1.0 - u_old, is_st_cu);
          } else {
          ...
          Real x_ratio = 0; // fraction of x_tmp/x_old
          if (x_old < small_value_30) {
            x_new = 0.0;
            x_ratio = 0.0;
          } else {
            // fraction of precabx and precabx_base
            Real u_new = utils::min_max_bound(0.0, 1.0, precabs_tmp_vec[k] / precabs_base_vec[k]);
            u_new = haero::min(u_new, u_old);
            if (mam_prevap_resusp_optcc <= 130) {
              // non-linear resuspension of aerosol mass
              x_new = 1.0 - faer_resusp_vs_fprec_evap_mpln(1.0 - u_new, is_st_cu);
          ...
          // aerosol mass resuspension
          scavabs = haero::max(0.0, scavabs * x_ratio);
        } else {
          // number resuspension
          scavabs = 0;
        }
      ...
      if (mam_prevap_resusp_optcc <= 130) 
        scavabs = haero::max(0.0, scavabs + tmpd[k]);
      }
      if (mam_prevap_resusp_optcc == 0) {
        Real fracev_st = 0;
        compute_evap_frac(mam_prevap_resusp_optcc, pdel[k], evapr[k], precabs_vec[k], fracev_st); 
        scavabs = scavabs * (1 - fracev_st) + srcs[k] * pdel[k] / gravit;
      }
      scavabs_vec[k+1] = scavabs;
    }
    ...
    for (int k = 0; k < nlev-1; ++k) {
      ...
      const Real small_value_30 = 1.e-30;
      Real scavabc = scavabc_vec[k];
      if (mam_prevap_resusp_optcc >= 100) {
        scavabc = scavabc_vec[k];
      if (precabc_vec[k] < small_value_30) {
	if (mam_prevap_resusp_optcc <= 130) 
	  scavabc = 0.0;
      } else if (evapr[k] <= 0.0) {
       // no evap so no resuspension
       if (mam_prevap_resusp_optcc <= 130) 
        ...
        if (mam_prevap_resusp_optcc <= 130) {
          // fraction of precabx and precabx_base
          const Real u_old =
              utils::min_max_bound(0.0, 1.0, precabc_vec[k] / precabc_base_vec[k]);
          // fraction after calling function *_resusp_vs_fprec_evap_mpln
          Real x_old = 0, x_new = 0;
          if (mam_prevap_resusp_optcc <= 130) {
            // non-linear resuspension of aerosol mass
          ...
          if (x_old < small_value_30) {
            ...
            // fraction of precabx and precabx_base
            Real u_new = utils::min_max_bound(0.0, 1.0, precabc_vec[k] / precabc_base_vec[k]);
            u_new = haero::min(u_new, u_old);
            if (mam_prevap_resusp_optcc <= 130) {
              // non-linear resuspension of aerosol mass
              x_new = 1.0 - faer_resusp_vs_fprec_evap_mpln(1.0 - u_new, is_st_cu);
          ...
          // aerosol mass resuspension
          scavabc = haero::max(0.0, scavabc_vec[k] * x_ratio);
        } else {
          // number resuspension
          scavabc = 0;
        }
      ...
      if (mam_prevap_resusp_optcc <= 130) 
        scavabc = haero::max(0.0, scavabc + tmpd[k]);
      }
      if (mam_prevap_resusp_optcc == 0) {
        Real fracev_cu = 0;
        compute_evap_frac(mam_prevap_resusp_optcc, pdel[k], evapc[k], precabc_vec[k+1], fracev_cu); 
        scavabc = scavabc * (1 - fracev_cu) + srcc[k] * pdel[k] / gravit;
      }
      scavabc_vec[k+1] = scavabc;
    }


    for (int k = 0; k < nlev; ++k) {
      const Real gravit = Constants::gravity;
      tmpa[k] = haero::max(0.0, evapc[k] * pdel[k] / gravit);
      tmpb[k] = haero::max(0.0, cmfdqr[k] * pdel[k] / gravit);
      tmpc[k] = (cmfdqr[k] - evapc[k]) * pdel[k] / gravit;
    }
    if (mam_prevap_resusp_optcc >= 100) {
      for (int k = 0; k < nlev-1; ++k) {
        const Real small_value_30 = 1.e-30;
        Real precabc_tmp = utils::min_max_bound(0.0, precabc_base_vec[k], precabc_vec[k] - tmpa[k]); 
        precabx_base_tmp[k] = precabc_base_vec[k];
        if (precabc_tmp < small_value_30) {
  	  // setting both these precip rates to zero causes the resuspension
          // calculations to start fresh if there is any more precip production
	  precabx_base_tmp[k] = 0;
	  precabc_tmp = 0;
        }
        precabc_base_vec[k+1] = haero::max(0.0, precabx_base_tmp[k] + tmpb[k]);
        precabc_vec[k+1] = utils::min_max_bound(0.0, precabc_base_vec[k+1], precabc_tmp + tmpb[k]);
      }
    } else if (mam_prevap_resusp_optcc == 0) 
      for (int k = 0; k < nlev-1; ++k) {
        precabc_vec[k+1] = precabc_vec[k] + tmpc[k];
    }
    ...
    if (mam_prevap_resusp_optcc >= 100 && mam_prevap_resusp_optcc > 130) {
      ...
      for (int k = 0; k < nlev-1; ++k) {
        ...
        const Real small_value_30 = 1.e-30;
        // raindrop number increase
        if (precabc_base_vec[k+1] < small_value_30) {
          precnumc_base_vec[k+1] = 0.0;
        } else if (precabc_base_vec[k+1] > precabx_base_tmp[k]) {
          // note - calc rainshaft number flux from rainshaft water flux,
          // then multiply by rainshaft area to get grid-average number flux
          const Real arainx = haero::max(cldvst[k+1], small_value_2);
          const Real tmpa = arainx * flux_precnum_vs_flux_prec_mpln(precabc_base_vec[k+1] / arainx, 2);
          precnumc_base_vec[k+1] = haero::max(0.0, tmpa);
        } else {
          precnumc_base_vec[k+1] = precnumc_base_vec[k];
        }
      }
    }
...
//std::cout<<__FILE__<<":"<<__LINE__<<" "<<k<<" "<<precabs_vec[k] << " " <<precabs<<std::endl;
if (precabs_vec[k] != precabs) {
  std::cout<<__FILE__<<":"<<__LINE__<<" **** ERROR *** "<<k<<" "<<precabs_vec[k] << " " <<precabs<<std::endl;
  Kokkos::abort("precabs_vec[k] != precabs");
}
if (precabs_base_vec[k] != precabs_base) {
  std::cout<<__FILE__<<":"<<__LINE__<<" **** ERROR *** "<<k<<" "<<precabs_base_vec[k] << " " <<precabs_base<<std::endl;
  Kokkos::abort("precabs_base_vec[k] != precabs_base");
}
if (precabc_vec[k] != precabc) {
  std::cout<<__FILE__<<":"<<__LINE__<<" **** ERROR *** "<<k<<" "<<precabc_vec[k] << " " <<precabc<<std::endl;
  Kokkos::abort("precabc_vec[k] != precabc");
}
if (precabc_base_vec[k] != precabc_base) {
  std::cout<<__FILE__<<":"<<__LINE__<<" **** ERROR *** "<<k<<" "<<precabc_base_vec[k] << " " <<precabc_base<<std::endl;
  Kokkos::abort("precabc_base_vec[k] != precabc_base");
}
//std::cout<<__FILE__<<":"<<__LINE__<<" "<<k<<" "<<precnums_base_vec[k] << " " <<precnums_base<<std::endl;
if (precnums_base_vec[k] != precnums_base) {
  std::cout<<__FILE__<<":"<<__LINE__<<" **** ERROR *** "<<k<<" "<<precnums_base_vec[k] << " " <<precnums_base<<std::endl;
  Kokkos::abort("precnums_base_vec[k] != precnums_base");
}
if (precnumc_base_vec[k] != precnumc_base) {
  std::cout<<__FILE__<<":"<<__LINE__<<" **** ERROR *** "<<k<<" "<<precnumc_base_vec[k] << " " <<precnumc_base<<std::endl;
  Kokkos::abort("precnumc_base_vec[k] != cprecnumc_base");
}
if (scavabs_vec[k] != scavabs) {
  std::cout<<__FILE__<<":"<<__LINE__<<" **** ERROR *** "<<k<<" "<<scavabs_vec[k] << " " <<scavabs<<std::endl;
  Kokkos::abort("scavabs_vec[k] != scavabs");
}
if (scavabc_vec[k] != scavabc) {
  std::cout<<__FILE__<<":"<<__LINE__<<" **** ERROR *** "<<k<<" "<<scavabc_vec[k] << " " <<scavabc<<std::endl;
  Kokkos::abort("scavabc_vec[k] != scavabc");
}
...
//std::cout<<__FILE__<<":"<<__LINE__<<" "<<k<<" "<<precabs_base_vec[k] << " " <<precabs_base<<std::endl;
if (precabs_vec[k] != precabs) {
  std::cout<<__FILE__<<":"<<__LINE__<<" precabs **** ERROR *** "<<k<<" "<<precabs_vec[k] << " " <<precabs<<std::endl;
  Kokkos::abort("precabs_vec[k] != precabs");
}
if (precabs_base_vec[k] != precabs_base) {
  std::cout<<__FILE__<<":"<<__LINE__<<" **** ERROR *** "<<k<<" "<<precabs_base_vec[k] << " " <<precabs_base<<std::endl;
...
if (precabc_vec[k] != precabc) {
  std::cout<<__FILE__<<":"<<__LINE__<<" precabc **** ERROR *** "<<k<<" "<<precabc_vec[k] << " " <<precabc<<std::endl;
  Kokkos::abort("precabc_vec[k] != precabc");
}
if (precabc_base_vec[k] != precabc_base) {
  std::cout<<__FILE__<<":"<<__LINE__<<" **** ERROR *** "<<k<<" "<<precabc_base_vec[k] << " " <<precabc_base<<std::endl;
...
if (precnumc_base_vec[k] != precnumc_base) {
  std::cout<<__FILE__<<":"<<__LINE__<<" **** ERROR *** "<<k<<" "<<precnumc_base_vec[k] << " " <<precnumc_base<<std::endl;
  Kokkos::abort("precnumc_base_vec[k] != precnumc_base");
}
//std::cout<<__FILE__<<":"<<__LINE__<<" "<<k<<" "<<scavabs_vec[k] << " " <<scavabs<<std::endl;
if (scavabs_vec[k] != scavabs) {
  std::cout<<__FILE__<<":"<<__LINE__<<" **** ERROR *** scavabs "<<k<<" "<<scavabs_vec[k] << " " <<scavabs<<std::endl;
  Kokkos::abort("scavabs_vec[k] != scavabs");
}
//std::cout<<__FILE__<<":"<<__LINE__<<" "<<k<<" "<<scavabc_vec[k] << " " <<scavabc<<std::endl;
if (scavabc_vec[k] != scavabc) {
  std::cout<<__FILE__<<":"<<__LINE__<<" **** ERROR *** scavabc "<<k<<" "<<scavabc_vec[k] << " " <<scavabc<<std::endl;
  Kokkos::abort("scavabc_vec[k] != scavabc");
}
