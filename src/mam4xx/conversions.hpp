...
/// @param [in] molecular_wt The molecular weight of the species/mixture
/// [kg/kmol]
KOKKOS_INLINE_FUNCTION Real mmr_from_vmr(const Real vmr,
                                         const Real molecular_wt) {
...
/// @param [in] molecular_wt The molecular weight of the species/mixture
/// [kg/kmol]
KOKKOS_INLINE_FUNCTION Real vmr_from_mmr(const Real mmr,
                                         const Real molecular_wt) {
  const auto mw_dry_air = Constants::molec_weight_dry_air;
...
KOKKOS_INLINE_FUNCTION Real vapor_saturation_pressure_hardy(Real T) {
  ...
  Real log_es = 0.0;
  // First one is T inverse squared.
  log_es += g[0] * 1 / (T * T);
  // Second one is just T inverse.
  log_es += g[1] * 1 / T;
  // Third one is T to the zero.
  log_es += g[2];
  // Fourth one is T.
  log_es += g[3] * T;
