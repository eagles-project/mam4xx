...
void vert_interp(const ThreadTeam &team, const int levsiz, const int pver,
                 const ConstView1D &pin, const ConstView1D &pmid,
                 const ConstView1D &datain, const View1D &dataout,
  ...
  Kokkos::parallel_for(Kokkos::TeamVectorRange(team, pver), [&](int k) {
    ...
    for (int kk = 0; kk < levsiz - 1; ++kk) {
      if (pin(kk) * unit_factor_pin < pmid(k) &&
  ...
  Kokkos::parallel_for(Kokkos::TeamVectorRange(team, ntrg), [&](int i) {
    ...
    if (tl < src_x(nsrc)) {
      ...
      for (int si = sil; si <= siu; ++si) {
        int si1 = si - 1;
        Real sl = max(tl, src_x(si1));
        Real su = min(tu, src_x(si));
        y += (su - sl) * src(si1);
      }
