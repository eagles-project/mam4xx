...
void soa_equilib_mixing_ratio_no_solute(const Real &T_in_K,      // in
  ...
  // g0_soa(ntot_soaspec)   ambient soa gas equilib mixing ratio (mol/mol at actual mw)
  ...
  const Real p0_soa = p0_soa_at_298K * exp( -( delh_vap_soa/(r_universal/1.e3) ) * 
...
//===============================================================================================
KOKKOS_INLINE_FUNCTION
void mam_soaexch_advance_in_time(const Real dt_full,        // in host model dt (s)
                                 const Real dt_sub_fixed,   // in fixed sub-step. A negative value means using adaptive step sizes
				 const int  niter_max,      // in maximum number of substeps
				 const Real alpha_astem,    // in error control parameter for sub-timesteps
				 const Real uptkaer[AeroConfig::num_gas_ids()][AeroConfig::num_modes()],  // in  uptake rate coefficient
				 ...
				 Real qaer_cur[AeroConfig::num_aerosol_ids()][AeroConfig::num_modes()], // inout
...
//static constexpr int ntot_poaspec = 1;
  ...
  // - At the beginning of each substep, a_soa and g_soa get their values from qaer_cur and qgas_cur but 
  //   with negative values set to zero.
  //- At the end of each substep, the values of a_soa and g_soa are copied back to qaer_cur and qgas_cur.

  Real a_soa[ntot_soaspec][max_mode];  // evolving SOA aerosol mixrat (mol/mol at actual mw), part of the unknowns of the ODEs
  Real g_soa[ntot_soaspec];            // evolving SOA gas     mixrat (mol/mol at actual mw), part of the unknowns of the ODEs

  Real qgas_prv[max_gas];      // gas mixrat at beginning of substep, saved for calculating time average of qgas
  Real qgas_avg_sum[max_gas];  // qgas*dt integrated over a time step, used for calculating time average of qgas
...
//const Real  eps_gas = 1.0e-20; // epsilon to be used on denominator for avoiding division by zero
  ...
  // -------------------------------------
  niter = 0;
  Real tcur  = 0.0;    // time elapsed since the beginning of sub-cycling (last full timestep)

  Real dtsum_qgas_avg = 0.0;  // total dt of all substeps, used for calculating time average of qgas
  for (int i=0; i<ntot_soaspec; ++i) 
    qgas_avg_sum[i] = 0.0;

  while (tcur < dt_full-eps_dt ) {
    ++niter;
    if (niter > niter_max) break;
    ...
    // ------------------------------------------------------------------------------
    //  Load incoming SOA gas into temporary array, force values to be non-negative
...
void mam_soaexch_1subarea(
    ...
    const Real dt_sub_soa_fixed,    // in fixed sub-step in s. A negative value means using adaptive step sizes
    ...
    const Real uptkaer[AeroConfig::num_gas_ids()][AeroConfig::num_modes()], // in
    const Real qaer_poa[1][AeroConfig::num_modes()],         // in POA mixing ratio (mol/mol at actual mw)
    Real qgas_cur[AeroConfig::num_gas_ids()],                // inout
    Real qgas_avg[AeroConfig::num_gas_ids()],                // inout
    Real qaer_cur[AeroConfig::num_aerosol_ids()][AeroConfig::num_modes()], // inout
    int &niter, // out
  ...
  // ntot_poaspec is the number of reacting gas species. The code only supports one
  // but MAM4 had an initial support for more so it was decided to keep
  // the form of the multi-species code if not the function.
  static constexpr int ntot_soamode = 1;
  static constexpr int ntot_poaspec = 1;
...
//static constexpr Real alpha_astem     = 0.05; // parameter used in calc of sub-step sizes
  ...
  // (This is needed to account for the solute effect on SOA's equilibrium mixing ratio.)
  // --------------------------------------------------------------------------------------
  ...
  // For primary carbon mode, keep the option to set opoa_frac=0 for consistency with older code
  ...
  // The calculation uses the oxygenation fraction specified above and the current (old) POA mixing ratios.
