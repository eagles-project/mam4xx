...
/*
KOKKOS_INLINE_FUNCTION
void sethet(const ColumnView het_rates[gas_pcnst], //[pver][gas_pcnst],  rainout
loss rates [1/s] //out ColumnView press,           // pressure [pascals] //in
    ...
    Real stay;          // fraction of layer traversed by falling drop in
timestep delt Real xdtm;           // the traveling time in each dz [s] Real
xxx2, xxx3;     // working variables for h2o2 (2) and so2 (3) Real yso2, yh2o2;
// washout lifetime [s] Real rlat;    // latitude in radians for columns Real
work1, work2;   // working variables Real t_factor;      // temperature factor
to calculate henry's law parameters Real xk0;           // working variable Real
zsurf;         // surface height [km] Real so2_diss;     // so2 dissociation
constant ColumnView xgas2, xgas3  // gas phase species for h2o2 (2) and so2 (3)
[molecules/cm^3] ColumnView delz;               // layer depth about interfaces
[cm] ColumnView xh2o2;              // h2o2 concentration [molecules/cm^3]
    ColumnView xso2;               // so2 concentration [molecules/cm^3]
    ColumnView xliq;               // liquid rain water content in a grid cell
[gm/m^3] ColumnView rain;               // precipitation (rain) rate
[molecules/cm^3/s] ColumnView precip;             // precipitation rate
[kg/kg/s] ColumnView xhen_h2o2, xhen_hno3, xhen_so2;   // henry law constants
...
#include "../yaml/mo_sethet/f90_yaml/sethet_beg_yml.f90"
    ...
    //-----------------------------------------------------------------
    xkgm = xdg/xrm * 2. + xdg/xrm * .6 * sqrt( xrm*xum/xvv ) *
(xvv/xdg)**(1./3.)
    ...
    do kk = 1,pver
       ...
       xliq(:ncol,kk)   = precip(:ncol,kk) * delt * xhnm(:ncol,kk) /
avo*mass_air * m3_2_cm3 xh2o2(:ncol,kk)  = qin(:ncol,kk,spc_h2o2_ndx) *
xhnm(:ncol,kk) xso2(:ncol,kk)  = qin(:ncol,kk,spc_so2_ndx) * xhnm(:ncol,kk)
    enddo
    ...
    col_loop :  do icol = 1,ncol
       ...
       level_loop1  : do kk = ktop(icol),pver
          ...
          //                        dt = dz(cm)/um(cm/s)
          //-----------------------------------------------------------------
          xdtm = delz(icol,kk) / xum                     // the traveling time
in each dz

          xxx2 = (xh2o2(icol,kk) - xgas2(kk))
          if( xxx2 /= 0. ) then                       // if no washout lifetime
= 1.e29 yh2o2  = xh2o2(icol,kk)/xxx2 * xdtm else yh2o2  = large_value_lifetime
          endif
          tmp_hetrates(icol,kk,2) = max( 1. / yh2o2,0. ) * stay

          xxx3 = (xso2( icol,kk) - xgas3(kk))
          if( xxx3 /= 0. ) then                       // if no washout lifetime
= 1.e29 yso2  = xso2( icol,kk)/xxx3 * xdtm else yso2  = large_value_lifetime
          endif
    ...
    level_loop2 : do kk = ktop_all,pver
       Column_loop2 : do icol=1,ncol
          ...
          if( h2o2_ndx > 0 ) then
             call calc_het_rates(satf_h2o2, rain(icol,kk), xhen_h2o2(icol,kk),&
// in tmp_hetrates(icol,kk,2), work1, work2,& // in het_rates(icol,kk,h2o2_ndx))
// out endif
          ...
          elseif( so2_ndx > 0 ) then
             call calc_het_rates(satf_so2, rain(icol,kk), xhen_so2(icol,kk),  &
// in tmp_hetrates(icol,kk,3), work1, work2,& // in het_rates(icol,kk,so2_ndx))
// out endif

          if( h2so4_ndx > 0 ) then
             call calc_het_rates(satf_hno3, rain(icol,kk), xhen_hno3(icol,kk), &
// in tmp_hetrates(icol,kk,1), work1, work2, & // in
...
number : '//hetratestrg) endif enddo #include
"../yaml/mo_sethet/f90_yaml/sethet_end_yml.f90" } // sethet
*/
