...
void form_gcm_of_gases_and_aerosols_from_subareas(
  ...
  for (int jsub = 1; jsub <= nsubarea; ++jsub) {
    for (int icnst = 0; icnst < gas_pcnst; ++icnst) {
      std::cout << "jsub = " << jsub << "\n";
      std::cout << "icnst = " << icnst << "\n";
      std::cout << "qsub[icnst][jsub] = " << qsub[icnst][jsub] << "\n";
      std::cout << "afracsub[jsub] = " << afracsub[jsub] << "\n";
      std::cout << "before qgcm[icnst] = " << qgcm[icnst] << "\n";
      qgcm[icnst] += qsub[icnst][jsub] * afracsub[jsub];
      std::cout << "after qgcm[icnst] = " << qgcm[icnst] << "\n";
    }
  }

  for (int icnst = 0; icnst < gas_pcnst; ++icnst) {
    std::cout << "before qgcm[icnst] = " << qgcm[icnst] << "\n";
    std::cout << "qgcm[icnst] = haero::max(0, qgcm[icnst]);" << "\n";
  ...
  if (ncldy_subarea <= 0) {
    for (int icnst = 0; icnst < gas_pcnst; ++icnst) {
      std::cout << "icnst = " << icnst << "\n";
      std::cout << "before qqcwgcm[icnst] = " << qqcwgcm[icnst] << "\n";
      std::cout << "qqcwgcm_old[icnst] = " << qqcwgcm_old[icnst] << "\n";
      std::cout << "qqcwgcm[icnst] = qqcwgcm_old[icnst];" << "\n";
      qqcwgcm[icnst] = qqcwgcm_old[icnst];
      std::cout << "after qqcwgcm[icnst] = " << qqcwgcm[icnst] << "\n";
    }
  } else {
    assign_1d_array(gas_pcnst, zero, // in
                    qqcwgcm);        // out
    for (int jsub = 1; jsub <= nsubarea; ++jsub) {
      for (int icnst = 0; icnst < gas_pcnst; ++icnst) {
        std::cout << "jsub = " << jsub << "\n";
        std::cout << "icnst = " << icnst << "\n";
        std::cout << "qqcwsub[icnst][jsub] = " << qqcwsub[icnst][jsub] << "\n";
        std::cout << "afracsub[jsub] = " << afracsub[jsub] << "\n";
        std::cout << "before qqcwgcm[icnst] = " << qqcwgcm[icnst] << "\n";
        std::cout << "qqcwgcm[icnst] += qqcwsub[icnst][jsub] * afracsub[jsub];" << "\n";
